class Mapper:
    ...
    def _set_value_to_response(self, keys, value, response_dict):
        """This method sets a value to a key in the response dictionary based on the target_keys list."""
        if len(keys) == 1:
    ...
    def _get_target_value(self, keys, source_dict):
        """This method gets the target value from the source dictionary based on the keys list."""
        if len(keys) == 1:
